...
import {
    ...
    CheckSquare, FileText, Activity, Database, PlusSquare, Clock, Stamp, ClipboardList, List
} from 'lucide-react';
...
const sidebarConfig = {
    ...
    // 👤 COMPLIANCE OFFICER UI (Full CRUD Access)
    COMPLIANCE_OFFICER: [
        ...
        // ✅ This path triggers the page WITH the "Create/Edit/Delete" buttons
        { title: 'Audit Management', path: '/compliance/audit-management', icon: ClipboardList },
    ],

    // 👤 GOVT AUDITOR UI (Read Only & Approvals)
    GOVT_AUDITOR: [
        { title: 'Dashboard', path: '/dashboard/auditor', icon: LayoutDashboard },
        // ✅ This path triggers the READ ONLY list
...
const Sidebar = ({ role = 'STUDENT', user }) => {
    // Spring Security often passes roles like "ROLE_UNIV_ADMIN". 
    // This cleans it up so it strictly matches our dictionary above.
    const cleanRole = role.replace('ROLE_', '');

    // Fallback to student if a mapped role isn't found
    const navLinks = sidebarConfig[cleanRole] || sidebarConfig['STUDENT'];

    // Helper to grab first letter of name or email for the avatar
