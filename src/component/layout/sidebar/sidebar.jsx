...
import {
    ...
    CheckSquare, FileText, Activity, Database, PlusSquare, Clock, Stamp, ClipboardList, List, X
} from 'lucide-react';
 
const sidebarConfig = {
    // ... [Keep your existing sidebarConfig exactly as is]
    STUDENT: [
    ...
    COMPLIANCE_OFFICER: [
        ...
        { title: 'Audit Management', path: '/compliance/audit-management', icon: ClipboardList },
    ],
    GOVT_AUDITOR: [
        { title: 'Dashboard', path: '/dashboard/auditor', icon: LayoutDashboard },
        { title: 'Audit List', path: '/auditor/audits', icon: List },
        { title: 'Pending Audits', path: '/auditor/pending', icon: Clock },
        { title: 'Submit Approvals', path: '/auditor/approvals', icon: Stamp },
    ],
};
 
// 1. Add isOpen and onClose to your props
const Sidebar = ({ role = 'STUDENT', user, isOpen, onClose }) => {
    ...
    return (
        <>
            {/* Optional but recommended: A backdrop overlay that dims the screen on mobile when sidebar is open */}
            {isOpen && (
                <div
            ...
            {/* 2. Dynamically apply the 'sidebar-open' class based on state */}
            <aside className={`edugov-sidebar ${isOpen ? 'sidebar-open' : ''}`}>
                <div className="sidebar-header">
                    <h2 className="module-title">EduGov</h2>
                   
                    {/* Optional: Mobile close button (visible only on small screens) */}
                    <button
                        className="mobile-close-btn"
                        onClick={onClose}
                        style={{ display: 'none' }} // We'll show this via CSS on mobile
                    >
                        <X size={20} color="#fff" />
                    </button>
                        ...
                        {navLinks.map((link, index) => {
                            ...
                            return (
                                <NavLink
                                    key={index}
                                    to={link.path}
                                    onClick={onClose} // 3. Auto-close sidebar on mobile when a link is clicked
                                    className={({ isActive }) =>
