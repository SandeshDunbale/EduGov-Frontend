...
const StudentProfile = () => {
    ...
    const userId = localStorage.getItem('userId'); // The ID 65 from your login
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            ...
            try {
                ...
                // 1. Fetch Student Details (Matches StudentController @GetMapping("/user/{userId}"))
                const studentRes = await axios.get(`http://localhost:8002/students/user/${userId}`, config);
                setStudentInfo(studentRes.data);

                // 2. Fetch Documents (Matches DocumentController @GetMapping("/user/{userId}"))
                ...
                <div className="col-lg-4">
                    <div className="card shadow-sm border-0 p-4">
                        <div className="text-center mb-4">
                            ...
                            <h4 className="fw-bold mb-1">{studentInfo?.name || "Student Name"}</h4>
                            <p className="text-muted small">System ID: {userId}</p>
                            {getStatusBadge(studentInfo?.status)}
                        </div>

                        <div className="border-top pt-3">
                            <h6 className="fw-bold small text-muted text-uppercase mb-3">Info</h6>
                            <div className="mb-2 d-flex align-items-center">
                                <Mail size={16} className="me-2 text-primary" />
                                <span className="text-truncate">{studentInfo?.email || 'N/A'}</span>
                            </div>
                            <div className="mb-2 d-flex align-items-center">
                            ...
                            <div className="mb-2 d-flex align-items-center">
                                ...
                                <span>{studentInfo?.address || 'N/A'}</span>
                            </div>
                            <div className="d-flex align-items-center">
                                <Calendar size={16} className="me-2 text-primary" />
                                <span>{studentInfo?.dob || 'N/A'}</span>
                            </div>
                        </div>
                    </div>
                </div>
