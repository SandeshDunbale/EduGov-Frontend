  ...
  const handleDelete = async (id) => {
    toast((t) => (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        <span>Permanently delete this compliance record?</span>
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '4px' }}>
          <button 
            style={{ background: '#f8fafc', border: '1px solid #e2e8f0', padding: '4px 12px', borderRadius: '4px', cursor: 'pointer', color: '#334155' }} 
            onClick={() => toast.dismiss(t.id)}
          >
            Cancel
          </button>
          <button 
            style={{ background: '#0ea5e9', color: '#fff', border: 'none', padding: '4px 12px', borderRadius: '4px', cursor: 'pointer' }} 
            onClick={async () => {
              toast.dismiss(t.id);
              try {
