...
const AdminEnrollments = () => {
    ...
    const fetchEnrollments = async (status) => {
        ...
        setStatusMsg('');
        try {
            // ALWAYS fetch all records to bypass the backend Enum error
            const res = await EnrollmentAPI.getAll(); 
            const allData = res.data || [];

            // Filter the data locally in React
            if (status === 'ALL') {
                setEnrollments(allData);
            } else {
                const filteredData = allData.filter(e => e.status === status);
                setEnrollments(filteredData);
                
                // If the local filter results in an empty array, show the fallback message
                if (filteredData.length === 0) {
                    setStatusMsg('No records found.');
                }
            }
            setPage(1);
        } catch (err) {
